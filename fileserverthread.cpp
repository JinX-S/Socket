...
int Serve_Accept_Link()// 接受连接请求，返回一个新的socket(描述符)，这个新socket用于同连接的客户端通信
{
    	...
    	if(new_server_socket_fd < 0) { 
            ...
            std::cout << "accept a client from ip:" << inet_ntoa(client_addr.sin_addr) 
                << " port: "  << ntohs(client_addr.sin_port) << std::endl;
        }
	return 1;
}

int Send_File_Name(int connfd)  //接收需要发送的文件位置及文件名
{
...
// 多线程函数
void fun(int connfd) {
	Send_File_Name(connfd);
	Document_Send(connfd);	
...
int Link_Trans()
{
	...
	{ 
		Serve_Accept_Link();
		// thread多线程
		std::thread td(fun, new_server_socket_fd);
		td.detach();
	} 
	return 1;
}
